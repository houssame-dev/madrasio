...
A later Stage 6.5 lifecycle completed its hosted fixture and source verification but stopped before the shared bundle returned an artifact. The temporary operator orchestration invoked `age-keygen` directly before `createStagingRecoveryBundle`; an `ExternalToolError` at that boundary was not a `RecoveryError` and therefore collapsed to `RECOVERY_OPERATION_FAILED`. The same collapse is reproducible locally when the reviewed age binary is absent from `PATH`, while the shared non-empty bundle and restore pass when the reviewed tool directory and Windows pnpm launcher are supplied. The retained hosted evidence cannot prove which external-tool condition caused that one attempt, so no provider cause is inferred. Stage 6 orchestration now uses the shared ephemeral-age helper, which returns only the recipient and external identity path, cleans its generated workspace, and maps key-generation failures to `BACKUP_ENCRYPTION_FAILED` with the bounded `key_generation` phase and safe tool diagnostics. A future hosted run will therefore distinguish this boundary before entering bundle construction.
