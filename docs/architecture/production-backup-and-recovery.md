...
Status: Task 050 Stage 5 Production-to-isolated-local restore readiness. Stage 6 STAGING/local recovery acceptance remains complete through reconstructed evidence, and Task 050 Stage 4.2 has now accepted one real encrypted Production recovery point with immutable R2 upload, remote verification, independent ciphertext readback, successful monitoring, and cleanup evidence. Recurring Production backup automation remains intentionally disabled. Customer onboarding remains blocked until the timed Production restore and remaining Production backup-cadence gates are accepted.

## Recovery objective and authority

The accepted target is an application-controlled recovery point at least every six hours (RPO target: 6 hours) and a rehearsed restoration within eight hours (RTO target: 8 hours). Stage 6 proves the repository-owned restore mechanism and its data, Auth identity, relational, migration, and application-security reconciliation against a real STAGING recovery bundle restored into an isolated local target. Task 050 Stage 4.2 now proves that a real encrypted Production recovery point can be created, uploaded immutably, verified remotely, independently read back, and reported successfully. That single accepted point proves Production recovery-point existence, but the six-hour Production RPO objective remains unproven while recurring Production backup automation is intentionally disabled and no operating cadence has yet been accepted. The RTO target remains unproven as a Production operational objective until the Stage 5 timed Production-to-isolated-local restore completes.
...
Stage 4.1 repository tests use deterministic fakes and local non-secret fixtures and perform no database, R2, Cronitor, Supabase, Vercel, or GitHub configuration access. Stage 4.2 subsequently accepted the real Production encrypted recovery-point creation, immutable upload, remote verification, independent readback, monitoring, and cleanup path. The remaining real restore proof belongs to Task 050 Stage 5: the accepted Production ciphertext must be restored into a fresh isolated local Supabase-compatible target under the reviewed pinned-tool and managed-Auth foundation, and application migrations must never create or own `auth.users`.
