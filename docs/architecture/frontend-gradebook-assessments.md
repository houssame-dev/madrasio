...
Authorized School administrators and Teachers with `grades.enter` can open the Gradebook create form. For Teachers, Year, Class, and Subject options are composed from the authenticated User's ACTIVE Teacher profiles and exact ACTIVE TeacherAssignments; a Teacher with no active assignment receives no create action. Academic Year, Academic Period, Class, Subject, and GradingConfigurationVersion all require explicit selection; changing Year clears any stale Period, Class, and Subject selection, and changing Class clears Subject. The version selector traverses every bounded page of `/api/v1/grading-configuration-versions` and labels each exact option with its configuration name and version number. It never accepts a UUID input, automatically chooses the sole option, or describes any option as “latest,” “current,” recommended, or default.
