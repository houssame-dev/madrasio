...
async function main(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      const actualTimestamps = migrations.rows.map((row) => row.created_at);
      if (JSON.stringify(actualTimestamps) !== JSON.stringify(expectedTimestamps)) {
        throw new Error('The hosted migration journal does not match the repository journal.');
      }
      if (tables.rows[0]?.count !== EXPECTED_TABLE_COUNT || !auth.rows[0]?.exists) {
        throw new Error('The hosted schema inventory does not match the reviewed STAGING schema.');
      }
      if (
  ...
  console.info(
    JSON.stringify({
      ...
      projectRef: process.env.DEPLOY_EXPECTED_PROJECT_REF,
...
main().catch((error) => {
  console.error(
    JSON.stringify({
      event: 'staging_migration_verification_failed',
