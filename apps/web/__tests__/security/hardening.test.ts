...
import { resolveSupabaseKeys, withSupabaseKeyCompatibility } from '@/lib/config/supabase-keys';
...
describe('Task 047 security contracts', () => {
  it('new-only key configuration needs no legacy aliases', () => {
    ...
    expect(withSupabaseKeyCompatibility({ NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY: 'public-fixture', SUPABASE_SECRET_KEY: 'private-fixture' }))
      .toMatchObject({ SUPABASE_ANON_KEY: 'public-fixture', SUPABASE_SERVICE_ROLE_KEY: 'private-fixture' });
  });
  it('preferred keys win during rolling transition; fallback is configuration-only', () => {
    expect(resolveSupabaseKeys({ NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY: 'new-public', SUPABASE_ANON_KEY: 'old-public', SUPABASE_SECRET_KEY: 'new-secret', SUPABASE_SERVICE_ROLE_KEY: 'old-secret' }))
      .toEqual({ publishableKey: 'new-public', secretKey: 'new-secret' });
    expect(resolveSupabaseKeys({ SUPABASE_ANON_KEY: 'legacy-public', SUPABASE_SERVICE_ROLE_KEY: 'legacy-secret' }))
      .toEqual({ publishableKey: 'legacy-public', secretKey: 'legacy-secret' });
  ...
  it('refuses elevated credentials in public/session key configuration without echoing values', () => {
    ...
    expect(() => resolveSupabaseKeys({ SUPABASE_ANON_KEY: fixture })).toThrow('cannot be public');
  });
