...
export function AssessmentsPanel({ schoolId, gradebook, period, canManage }: {
  ...
  return <section className="space-y-4">
    ...
    {query.isError ? <ApiErrorState title={t.unavailable} onRetry={() => void query.refetch()} /> : <div className="space-y-4"><TableShell headers={[t.titleField, t.type, t.maximumScore, t.weight, t.date, t.status, t.actions]} loading={query.isPending}>{query.data?.data.length === 0 ? <EmptyTableRow columns={7} /> : query.data?.data.map((assessment) => <tr key={assessment.id}><td className="px-4 py-3 font-medium">{assessment.title}</td><td className="px-4 py-3">{t.typeLabels[assessment.assessmentType]}</td><td className="px-4 py-3 tabular-nums">{assessment.maximumScore}</td><td className="px-4 py-3 tabular-nums">{assessment.weight}</td><td className="px-4 py-3">{assessment.assessmentDate ?? t.none}</td><td className="px-4 py-3"><StatusBadge status={assessment.status} /></td><td className="px-4 py-3"><div className="flex justify-end gap-2">{canManage && mutableGradebook && assessment.status === 'DRAFT' ? <Button type="button" variant="ghost" size="sm" aria-label={`${t.editAssessment}: ${assessment.title}`} onClick={() => setEditing(assessment)}><Pencil className="size-4" aria-hidden="true" /></Button> : null}{canManage && mutableGradebook ? <AssessmentLifecycleActions assessment={assessment} onSaved={() => changed()} /> : null}</div></td></tr>)}</TableShell>{query.data ? <Pagination {...query.data.meta} onPage={(next) => updateUrl({ assessmentPage: next })} /> : null}</div>}
    <p className="text-sm text-muted-foreground">{t.futureGradeEntry}</p>
